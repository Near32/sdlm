"""
Sweep logits_top_k and measure final LCS ratios after 1-epoch optimize_inputs calls.
...
def _parse_top_k_values(raw: str) -> List[int]:
    ...
    if not vals:
        raise ValueError("top_k_values is empty")
    for v in vals:
        if v <= 0:
            raise ValueError(f"Invalid top_k value {v}; expected top_k > 0")
    return vals
...
def _plot_records(records: List[Dict], out_png: Path, title: str) -> None:
    if not records:
        return
    recs = sorted(records, key=lambda r: r["top_k"])
    xs = [r["top_k"] for r in recs]
...
def _save_records(records: List[Dict], out_dir: Path, prefix: str) -> Dict[str, str]:
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f, fieldnames=["top_k", "original_lcs_ratio", "discrete_lcs_ratio", "embsim_lcs_ratio"]
        )
        writer.writeheader()
...
def _run_single_target_sweep(
    ...
    for idx, top_k in enumerate(top_k_values):
        ...
        # Compute superposition diagnostics once per target (first top_k call only).
        ...
        temps_path = temps_dir / f"learnable_temperatures_{target_key}_top_k_{top_k_tag}.pt"
        torch.save(final_temps, temps_path)
        temperature_paths.append({"top_k": int(top_k), "path": str(temps_path)})

        if final_temps:
            temps_artifact = wandb.Artifact(
                name=f"learnable-temperatures-{target_key}-tk-{top_k_tag}",
                type="tensor",
                description="Final learnable temperatures from top-k sweep call",
            )
        ...
        run.log(
            {
                f"top_k_sweep/{target_key}/top_k": rec["top_k"],
                f"top_k_sweep/{target_key}/original_lcs_ratio": rec["original_lcs_ratio"],
                f"top_k_sweep/{target_key}/discrete_lcs_ratio": rec["discrete_lcs_ratio"],
                f"top_k_sweep/{target_key}/embsim_lcs_ratio": rec["embsim_lcs_ratio"],
                f"top_k_sweep/{target_key}/learnable_temperatures_path": str(temps_path),
                f"top_k_sweep/{target_key}/fallback_forced_non_learnable_temperature": int(force_non_learnable_temperatures),
                "top_k_sweep/target_key": target_key,
            },
            step=idx,
        )

    saved = _save_records(records, output_dir, prefix=f"top_k_lcs_sweep_{target_key}")
    _plot_records(
        records=records,
        out_png=Path(saved["png_path"]),
        title=f"LCS ratio vs logits_top_k ({target_key}, epochs=1)",
    )

    table = wandb.Table(columns=["top_k", "original_lcs_ratio", "discrete_lcs_ratio", "embsim_lcs_ratio"])
    for r in sorted(records, key=lambda x: x["top_k"]):
        table.add_data(r["top_k"], r["original_lcs_ratio"], r["discrete_lcs_ratio"], r["embsim_lcs_ratio"])
    run.log(
        {
            f"top_k_sweep/{target_key}/table": table,
            f"top_k_sweep/{target_key}/plot": wandb.Image(saved["png_path"]),
        }
    )
...
def run_sweep(args: argparse.Namespace) -> Dict:
    ...
    if args.dataset_path:
        ...
        with (output_dir / "top_k_lcs_sweep_summary.json").open("w", encoding="utf-8") as f:
            json.dump(summary, f, indent=2)
        run.summary["top_k_sweep_output_dir"] = str(output_dir.resolve())
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Sweep logits_top_k vs LCS ratios using 1-epoch optimize_inputs calls.")
    ...
    p.add_argument("--top_k_values", type=str, default="1,2,4,16,32,128,1024,4096")
    p.add_argument("--logits_top_p", type=float, default=1.0,
                   help="Fixed top-p used while sweeping logits_top_k (1.0 disables top-p filtering).")
    ...
    p.add_argument("--superposition_metric_every", type=int, default=0,
                   help="Run superposition z_i-z_j diagnostics every N epochs; in this sweep it is "
                        "executed once per target (first top_k call only) when >0.")
    ...
    p.add_argument("--wandb_project", type=str, default="prompt-optimization-top-k-sweep")
    p.add_argument("--wandb_entity", type=str, default=None)
    p.add_argument("--wandb_run_name", type=str, default="top_k_lcs_sweep")
    p.add_argument("--output_dir", type=str, default="top_k_lcs_sweep_results")
    return p
