"""
SODA optimization using HuggingFace transformers - internal batching variant.

This implements the SODA algorithm with dynamic batch queue management,
similar to the original SODA paper implementation.
"""
...
def soda_optimize_inputs_hf_batch(
    model,
    tokenizer,
    device: torch.device,
    ...
    """
    Multi-target SODA optimization using HuggingFace with internal batching.

    This variant manages a dynamic queue of targets, processing them in batches
    and removing targets as they converge (similar to the original SODA notebook).
    ...
    for target in targets:
        tokens = tokenizer(target["text"], return_tensors="pt").input_ids.to("cpu")
        target_tokens_list.append(tokens[0])
    ...
    pbar = tqdm(total=num_targets, desc="SODA Batch", position=0)
    completed_prev = 0
    ...
    while state.num_remain_items != 0 or len(state.batch_results) != 0:
        ...
        with torch.no_grad():
            ...
            if (max_batch_size - len(state.batch_results)) > 0 and state.num_remain_items != 0:
                ...
                for i in range(num_new_items):
                    ...
                    # (following original SODA's per-position optimization)
                    params = []
        ...
        target_embeds = embedding_layer(state.target_tokens[:, :-1])

        # Concatenate prompt and target embeddings
        combined_embeds = torch.cat([pred_embed, target_embeds], dim=1)
        ...
        target_pred_logits = pred_logits[:, seq_len - 1:seq_len - 1 + target_length, :]
        ...
        with torch.no_grad():
            ...
            pred_tokens_with_target = torch.cat(
                [pred_tokens, state.target_tokens[:, :-1]], dim=1
            )
            discrete_outputs = model(
                inputs_embeds=embedding_layer(pred_tokens_with_target),
                return_dict=True
            )
            discrete_logits = discrete_outputs.logits
            generated = discrete_logits[:, seq_len - 1:seq_len - 1 + target_length, :].argmax(dim=-1)
    ...
    for result in state.results:
        ...
        gen_text = tokenizer.decode(result["generated_tokens"], skip_special_tokens=False) if "generated_tokens" in result else ""
