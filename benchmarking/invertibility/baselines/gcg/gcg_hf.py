"""
GCG optimization using HuggingFace transformers - single-target variant.

This implements the GCG (Greedy Coordinate Gradient) algorithm for prompt
inversion using the HuggingFace model API.
"""
...
def gcg_optimize_inputs_hf(
    ...
    GCG works by:
    1. Computing gradients of the loss w.r.t. a one-hot embedding
    2. Using gradient information to select positions and candidate tokens
    3. Evaluating multiple candidates and keeping the best one
    ...
    for epoch in pbar:
        ...
        # Compute loss (log-probability difference as in original GCG)
        ...
        pbar.set_description(
            f"GCG Epoch {epoch + 1}, Loss: {loss.item():.4f}, LCS: {lcs_ratio:.4f}"
        )

        # Early stopping
        if early_stop_on_exact_match and is_exact_match:
            logger.info(f"GCG: Exact match found at epoch {epoch + 1}")
            wandb.log({"generated_output_table": copy.deepcopy(wandb_table)})
            break
        if lcs_ratio >= lcs_ratio_threshold:
            logger.info(f"GCG: LCS ratio threshold reached at epoch {epoch + 1}")
            wandb.log({"generated_output_table": copy.deepcopy(wandb_table)})
            break
    ...
    # Get final results
    with torch.no_grad():
        final_embed = embedding_layer(pred_tokens)
        target_embeds = embedding_layer(target_tokens[:, :-1])
